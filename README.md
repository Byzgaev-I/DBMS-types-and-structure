...
1. Данные записываются на все узлы с задержкой до часа (асинхронная запись): CAP=AP (Доступность + Устойчивость к разделению)
2. При сетевых сбоях, система может разделиться на 2 раздельных кластера: CAP=AP (Доступность + Устойчивость к разделению)
3. Условие доступности требует, чтобы запрос всегда завершался корректным ответом. CAP=CP 
...
3. Система может не прислать корректный ответ или сбросить соединение: PC/EC (Предпочтение Согласованности при разделении + Предпочтение Согласованности при отсутствии разделения)
